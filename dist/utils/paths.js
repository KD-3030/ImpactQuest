 ...
 */
export function getTemplatesPath(importMetaUrl) {
    ...
    const isInDist = currentDir.includes('/dist') || currentDir.endsWith('/dist') ||
        currentDir.includes('\\dist') || currentDir.endsWith('\\dist');
    let templatesPath;
    if (isInDist) {
 ...
 */
export function getPlopfilePath(importMetaUrl) {
    const currentDir = getDirname(importMetaUrl);
    ...
    if (isInDist) {
        ...
        // From src/generators/ go up to src/, then to plopfile.ts (during development)
